...
LaserScan::LaserScan() : private_node_("~")
{
    ...
    ROS_INFO_STREAM("\033[1;32m----> Feature Extraction StartedBrian Edited.\033[0m");
    ...
    feature_scan_publisher_ = node_handle_.advertise<sensor_msgs::LaserScan>("feature_scan11223", 1, this);
...
void LaserScan::ScanCallback(const sensor_msgs::LaserScan::ConstPtr &scan_msg)
{
    ...
    std::cout<<"invalid data number out of all: "<<invaliddata<<"/"<<scan_count<<std::endl;
    ...
    std::cout<<"Cost  "<< time_used.count() << " to process 1 scan data" << std::endl;
}
